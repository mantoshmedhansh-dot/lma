...
export function createMockQueryBuilder(
  ...
  builder.then = vi.fn((resolve) =>
    resolve({
      data: returnData,
      error: returnError,
      count: Array.isArray(returnData) ? returnData.length : null,
    }),
  );

  // Make it awaitable
  (builder as { data: unknown; error: unknown; count: number | null }).data =
    returnData;
  (builder as { data: unknown; error: unknown; count: number | null }).error =
    returnError;
  (builder as { data: unknown; error: unknown; count: number | null }).count =
    Array.isArray(returnData) ? returnData.length : null;

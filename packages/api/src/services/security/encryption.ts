...
export function anonymizeUserData(
  ...
  for (const field of piiFields) {
    if (anonymized[field]) {
      if (options.preserveFormat) {
        anonymized[field] = generateAnonymousValue(
          field,
          anonymized[field] as string,
        );
      } else {
  ...
  if (anonymized.user_id) {
    anonymized.anonymized_user_id = hashData(
      anonymized.user_id as string,
    ).slice(0, 16);
...
async function logDataExportRequest(
  userId: string,
  requestedAt: Date,
): Promise<void> {

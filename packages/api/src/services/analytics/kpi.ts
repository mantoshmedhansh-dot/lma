...
export async function getDeliveryKPIs(
  ...
  return {
    ...
    onTimeDeliveryRate:
      deliveryTimes.length > 0 ? (onTimeCount / deliveryTimes.length) * 100 : 0,
...
export async function getDriverKPIs(
  ...
  const { data: allDrivers } = await supabaseAdmin
    .from("drivers")
    .select(
      "id, user_id, status, average_rating, total_deliveries, is_active, users(full_name)",
    );
  ...
  // Calculate averages
  const ratings =
    allDrivers?.filter((d) => d.average_rating).map((d) => d.average_rating) ||
    [];
  const avgRating =
    ratings.length > 0
  ...
  const totalDeliveries = Object.values(driverStats).reduce(
    (sum, s) => sum + s.deliveries,
    0,
  );
  const avgDeliveriesPerDriver =
    driverCount > 0 ? totalDeliveries / driverCount : 0;
  ...
  const utilizationRate =
    activeDrivers > 0 ? (driverCount / activeDrivers) * 100 : 0;
...
export async function getMerchantKPIs(
  ...
  let merchantQuery = supabaseAdmin
    .from("merchants")
    .select(
      "id, business_name, category, average_rating, estimated_prep_time, is_active",
    );
  ...
  const merchantStats: Record<
    string,
    { orders: number; revenue: number; prepTimes: number[] }
  > = {};
  periodOrders?.forEach((order) => {
    if (order.merchant_id) {
      if (!merchantStats[order.merchant_id]) {
        merchantStats[order.merchant_id] = {
          orders: 0,
          revenue: 0,
          prepTimes: [],
        };
  ...
  const totalOrders = Object.values(merchantStats).reduce(
    (sum, s) => sum + s.orders,
    0,
  );
  const avgOrdersPerMerchant =
    merchantCount > 0 ? totalOrders / merchantCount : 0;
  ...
  // Average rating
  const ratings =
    merchants?.filter((m) => m.average_rating).map((m) => m.average_rating) ||
    [];
...
export async function getFinancialKPIs(
  ...
  const totalCommissions =
    orders?.reduce(
      (sum, o) => sum + (o.total_amount || 0) * commissionRate,
      0,
    ) || 0;
  ...
  orders?.forEach((o) => {
    ...
    revenueByPaymentMethod[method] =
      (revenueByPaymentMethod[method] || 0) + (o.total_amount || 0);
  });
  ...
  orders?.forEach((o) => {
    ...
    dailyRevenueMap[date] =
      (dailyRevenueMap[date] || 0) + (o.total_amount || 0);
  });
...
export async function getOperationalKPIs(
    ...
    if (o.driver_assigned_at && o.picked_up_at) {
      const wait =
        (new Date(o.picked_up_at).getTime() -
          new Date(o.driver_assigned_at).getTime()) /
        60000;
      if (wait > 0 && wait < 120) {
  ...
  const avgPickupWaitTime =
    pickupWaitTimes.length > 0
  ...
  const totalOrders = orders?.length || 0;
  const failedOrders =
    orders?.filter((o) => o.status === "failed" || o.status === "returned")
      .length || 0;
  ...
  const completedOrders =
    orders?.filter((o) => o.status === "delivered").length || 0;
  const returnRate =
    completedOrders > 0 ? (returnedOrders / completedOrders) * 100 : 0;
...
export async function getDashboardSummary(
  ...
  const [delivery, drivers, merchants, financial, operational] =
    await Promise.all([
      getDeliveryKPIs(period, options),
      getDriverKPIs(period, {
...
export async function compareKPIs(
  ...
  const changes: Record<string, { value: number; percentage: number }> = {
    ...
    completedOrders: calculateChange(
    ...
    avgDeliveryTime: calculateChange(

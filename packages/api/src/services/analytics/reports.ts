...
export async function generateReport(
  ...
  switch (config.type) {
    ...
    case "driver_performance":
      data = await generateDriverPerformanceReport(
        dateRange,
        config.filters?.driverId,
      );
      break;
    case "merchant_settlement":
      data = await generateMerchantSettlementReport(
        dateRange,
        config.filters?.merchantId,
      );
      break;
    case "financial_reconciliation":
      ...
      break;
    case "cod_collection":
      data = await generateCODCollectionReport(
        dateRange,
        config.filters?.driverId,
      );
      break;
    ...
    case "zone_performance":
      data = await generateZonePerformanceReport(
        dateRange,
        config.filters?.zoneId,
      );
      break;
    default:
 ...
 */
async function generateWeeklySummary(dateRange: {
  start: Date;
  end: Date;
}): Promise<{
  ...
  const merchantKPIs = await getMerchantKPIs("custom", {
    customRange: dateRange,
  });
  ...
  const dailyStatsMap: Record<
    string,
    { orders: number; revenue: number; completed: number }
  > = {};
  orders?.forEach((o) => {
  ...
  const dailyStats = Object.entries(dailyStatsMap)
    .map(([date, stats]) => ({
      ...
      completionRate:
        stats.orders > 0
          ? Math.round((stats.completed / stats.orders) * 1000) / 10
          : 0,
    }))
  ...
  return {
    ...
    summary: {
      ...
      avgDailyRevenue:
        Math.round((deliveryKPIs.totalRevenue / days) * 100) / 100,
      completionRate: Math.round(deliveryKPIs.completionRate * 10) / 10,
      avgDeliveryTime: deliveryKPIs.avgDeliveryTime,
    },
    dailyStats,
...
async function generateMonthlySummary(dateRange: { start: Date; end: Date }) {
  ...
  const financialKPIs = await getFinancialKPIs("custom", {
    customRange: dateRange,
  });
...
async function generateDriverPerformanceReport(
  ...
  const driverReport =
    drivers?.map((d) => {
      ...
      const avgTime =
        stats.deliveryTimes.length > 0
          ? stats.deliveryTimes.reduce((a, b) => a + b, 0) /
            stats.deliveryTimes.length
          : 0;
 ...
 */
async function generateFinancialReconciliation(dateRange: {
  start: Date;
  end: Date;
}) {
  const financial = await getFinancialKPIs("custom", {
    customRange: dateRange,
  });
  ...
  const paymentStatusBreakdown: Record<
    string,
...
async function generateCODCollectionReport(
  ...
  let query = supabaseAdmin
    .from("orders")
    .select(
      "id, driver_id, total_amount, status, delivered_at, drivers(users(full_name))",
    )
    .eq("is_cod", true)
    .gte("created_at", dateRange.start.toISOString())
    .lte("created_at", dateRange.end.toISOString());
  ...
  const totalCollected = Object.values(driverCOD).reduce(
    (sum, d) => sum + d.collected,
    0,
  );
  const totalPending = Object.values(driverCOD).reduce(
    (sum, d) => sum + d.pending,
    0,
  );
...
async function generateZonePerformanceReport(
  ...
  const zoneIds = zoneId
    ? [zoneId]
    : [...new Set(orders?.map((o) => o.zone_id) || [])];
...
function getReportDateRange(schedule: "daily" | "weekly" | "monthly"): {
  start: Date;
  end: Date;
} {
  const end = new Date();

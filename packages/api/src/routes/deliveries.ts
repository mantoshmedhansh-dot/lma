...
const podSchema = z.object({
  ...
  recipient_relationship: z
    .enum(["self", "family", "security", "neighbor", "other"])
    .optional(),
});
    ...
    try {
      ...
      if (podData.delivery_type === "handed" && !podData.signature_url) {
        throw new ApiError(
          400,
          "SIGNATURE_REQUIRED",
          "Signature is required for handed delivery",
        );
      }
      ...
      if (order.is_cod && !podData.cod_collected) {
        throw new ApiError(
          400,
          "COD_NOT_COLLECTED",
          "Cash on delivery must be collected",
        );
      }
      ...
      if (podError) {
        throw new ApiError(
          500,
          "POD_CREATION_FAILED",
          "Failed to create proof of delivery",
        );
      }
      ...
      // Update driver stats
      await supabaseAdmin.rpc("increment_driver_deliveries", {
        driver_id: driver.id,
      });
...
router.get(
  ...
  async (req, res, next) => {
    try {
      const { page, limit, status, from_date, to_date } = req.query as z.infer<
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const { collection_ids, deposit_reference, deposit_account } =
        req.body as z.infer<typeof codDepositSchema>;
    ...
    try {
      ...
      const transformedOrders = orders?.map(
        (order: Record<string, unknown>) => ({
          ...order,
          items_count:
            (order.order_items as Array<{ count: number }>)?.[0]?.count || 0,
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const { from, to, vehicleType } = req.body as z.infer<
        typeof etaRequestSchema
      >;
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const { pickup, delivery, prepTime, vehicleType } = req.body as z.infer<
        typeof deliveryEstimateSchema
      >;
    ...
    try {
      ...
      if (!driver.current_latitude || !driver.current_longitude) {
        throw new ApiError(
          400,
          "LOCATION_REQUIRED",
          "Please enable location to get route",
        );
      }
      ...
      for (const order of orders) {
        ...
        if (["driver_assigned"].includes(order.status)) {
          locations.push({
            ...
            orderId: order.id,
            address:
              (order.pickup_address_snapshot as Record<string, string>)
                ?.address_line_1 || "",
            estimatedTime: 5,
          });
        }
        ...
        locations.push({
          ...
          orderId: order.id,
          address:
            (order.delivery_address_snapshot as Record<string, string>)
              ?.address_line_1 || "",
          estimatedTime: 5,
        });
      }
      ...
      const enrichedStops = optimizedRoute.stops.map((stop) => {
        ...
        return {
          ...
          merchantName: (order?.merchant as Record<string, string>)
            ?.business_name,

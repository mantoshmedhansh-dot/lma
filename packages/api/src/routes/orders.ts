    ...
    try {
      ...
      const transformedOrder = {
        ...
        merchant: {
          ...
          business_name: (order.merchants as Record<string, unknown>)
            ?.business_name,
        ...
        driver: order.drivers
          ? {
              ...
              phone: (
                (order.drivers as Record<string, unknown>)?.users as Record<
                  string,
                  unknown
                >
              )?.phone,
              ...
              vehicle_type: (order.drivers as Record<string, unknown>)
                ?.vehicle_type,
              vehicle_number: (order.drivers as Record<string, unknown>)
                ?.vehicle_number,
              current_latitude: (order.drivers as Record<string, unknown>)
                ?.current_latitude,
              current_longitude: (order.drivers as Record<string, unknown>)
                ?.current_longitude,
              average_rating: (order.drivers as Record<string, unknown>)
                ?.average_rating,
            }
          : null,
        delivery_address: {
        ...
        payment_method: (order.payments as Array<Record<string, unknown>>)?.[0]
          ?.payment_method,
        payment_status: (order.payments as Array<Record<string, unknown>>)?.[0]
          ?.status,
        ...
        if (item.addons) {
          for (const addonItem of item.addons) {
            const addon = product.product_addons?.find(
              (a: { id: string }) => a.id === addonItem.addon_id,
            );
            if (addon) {

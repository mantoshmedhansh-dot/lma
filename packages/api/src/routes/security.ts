...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await logDataAccess("viewed", req.user!.id, "audit_logs", "query", req, {
        query: req.query,
      });
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const { format, includeOrderHistory, includePayments, includeLocations } =
        req.body;
...
router.post("/consents", authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (!consentType || typeof granted !== "boolean") {
      return res
        .status(400)
        .json({ error: "Consent type and granted status required" });
    }
    ...
    types: [
      {
        ...
        description:
          "Receive promotional offers, discounts, and news about our services",
        required: false,
      },
      {
        id: "analytics",
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to apply retention policies" });
    }
  },
);

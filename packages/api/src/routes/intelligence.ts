 ...
 */
router.post(
  "/allocation/auto-assign/:orderId",
  authenticate,
  requireRole(["admin", "merchant"]),
  async (req: Request, res: Response) => {
    try {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!name || !type || !polygon?.coordinates) {
        return res
          .status(400)
          .json({ error: "Missing required fields: name, type, polygon" });
      }
 ...
 */
router.post(
  "/zones/check-serviceability",
  authenticate,
  async (req: Request, res: Response) => {
    try {
      const { latitude, longitude, merchantId, orderValue, vehicleType } =
        req.body;
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.error("Get drivers in zone error", {
        error,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!condition || !multiplier) {
        return res
          .status(400)
          .json({ error: "Missing required fields: condition, multiplier" });
      }
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const multiplier = await calculateSurgeMultiplier(zoneId, {
        latitude,
        longitude,
      });
      ...
      logger.error("Surge calculation error", {
        error,
        zoneId: req.params.zoneId,
      });
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (targetDate === undefined || targetHour === undefined) {
        return res
          .status(400)
          .json({ error: "targetDate and targetHour are required" });
      }

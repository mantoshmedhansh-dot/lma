...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const kpis = await getDriverKPIs(period as "today" | "week" | "month", {
        zoneId,
      });
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const kpis = await getMerchantKPIs(period as "today" | "week" | "month", {
        category,
      });
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!type || !schedule || !recipients?.length) {
        return res
          .status(400)
          .json({ error: "Type, schedule, and recipients are required" });
      }
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to get operational metrics" });
    }
  },
);
 ...
 */
router.post(
  "/live/drivers/:driverId/location",
  authenticate,
  requireRole(["driver"]),
  async (req: Request, res: Response) => {
    try {

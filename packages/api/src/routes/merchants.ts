...
const listQuerySchema = z.object({
  ...
  sort_by: z
    .enum(["rating", "distance", "name", "delivery_time"])
    .default("rating"),
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      // Transform response
      const transformedMerchants = merchants?.map(
        (merchant: Record<string, unknown>) => ({
          ...merchant,
          categories:
            (
              merchant.merchant_categories as Array<{ categories: unknown }>
  ...
  async (req, res, next) => {
    try {
      ...
      const transformedMerchant = {
        ...merchant,
        categories:
          merchant.merchant_categories?.map(
            (mc: { categories: unknown }) => mc.categories,
          ) || [],

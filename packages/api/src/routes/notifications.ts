  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!recipients || !payload) {
        return res
          .status(400)
          .json({ error: "Recipients and payload are required" });
      }
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.error("Failed to get live tracking", {
        error,
        orderId: req.params.orderId,
      });
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!orderId || !overallRating) {
        return res
          .status(400)
          .json({ error: "Order ID and overall rating required" });
      }
 ...
 */
router.get(
  "/ratings/merchant/:merchantId/stats",
  authenticate,
  async (req: Request, res: Response) => {
    try {
 ...
 */
router.get(
  "/ratings/driver/:driverId/recent",
  authenticate,
  async (req: Request, res: Response) => {
    try {
      const { driverId } = req.params;
 ...
 */
router.get(
  "/ratings/merchant/:merchantId/recent",
  authenticate,
  async (req: Request, res: Response) => {
    try {
      const { merchantId } = req.params;

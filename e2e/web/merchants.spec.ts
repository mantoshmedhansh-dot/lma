  ...
  test.describe("Home Page", () => {
    test("should display merchant listing", async ({ page }) => {
      ...
      const merchants = page.locator(
        '[data-testid="merchant-card"], [data-testid="category-card"]',
      );
    ...
    test("should filter merchants by search", async ({ page }) => {
      ...
      const resultsOrMessage = page.locator(
        '[data-testid="merchant-card"], [data-testid="no-results"]',
      );
    ...
    test("should have category filters", async ({ page }) => {
      ...
      const categories = page.locator(
        '[data-testid="category-filter"], [role="tablist"]',
      );
  ...
  test.describe("Search Results", () => {
    test("should display search results page", async ({ page }) => {
      ...
      const content = page.locator(
        '[data-testid="search-results"], [data-testid="no-results"], main',
      );
    ...
    test("should display search query", async ({ page }) => {
      ...
      await expect(page.getByText(/burger/i))
        .toBeVisible({ timeout: 10000 })
        .catch(() => {

...
test.describe("Super Admin Platform", () => {
  ...
  test.describe("Login", () => {
    ...
    test("should restrict access to non-admin users", async ({ page }) => {
      ...
      await expect(
        page
          .getByText(/unauthorized|access denied|invalid/i)
  ...
  test.describe("Dashboard", () => {
    test("should display admin dashboard", async ({ page }) => {
      ...
      await expect(dashboardContent.or(loginPage)).toBeVisible({
        timeout: 10000,
      });
    });
  ...
  test.describe("Merchant Management", () => {
    test("should display merchants page", async ({ page }) => {
      ...
      const merchantsContent = page.locator(
        'table, [data-testid="merchants-list"]',
      );
      const loginPage = page.locator('input[type="email"]');

      await expect(merchantsContent.or(loginPage)).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test("should display pending approvals count", async ({ page }) => {
      ...
      const pendingIndicator = page.locator(
        '[data-testid="pending-count"], :text("Pending")',
      );
  ...
  test.describe("Driver Management", () => {
    test("should display drivers page", async ({ page }) => {
      ...
      const driversContent = page.locator(
        'table, [data-testid="drivers-list"]',
      );
      const loginPage = page.locator('input[type="email"]');

      await expect(driversContent.or(loginPage)).toBeVisible({
        timeout: 10000,
      });
    });
  });
  ...
  test.describe("Analytics", () => {
    test("should display analytics page", async ({ page }) => {
      ...
      const analyticsContent = page.locator(
        '[data-testid="analytics"], h1:has-text("Analytics")',
      );
      const loginPage = page.locator('input[type="email"]');

      await expect(analyticsContent.or(loginPage)).toBeVisible({
        timeout: 10000,
      });
    });
    ...
    test("should have date range selector", async ({ page }) => {
      ...
      const dateSelector = page.locator(
        '[data-testid="date-range"], select:has-text("days")',
      );
  ...
  test.describe("Settings", () => {
    test("should display settings page", async ({ page }) => {
      ...
      const settingsContent = page.locator(
        'form, [data-testid="settings"], h1:has-text("Settings")',
      );
      const loginPage = page.locator('input[type="email"]');

      await expect(settingsContent.or(loginPage)).toBeVisible({
        timeout: 10000,
      });
    });

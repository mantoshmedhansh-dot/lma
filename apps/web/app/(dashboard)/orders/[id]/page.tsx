        ...
        <div className="grid gap-6 md:grid-cols-2">
            ...
            <CardContent className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2 pt-2">
                ...
                <div className="rounded border p-2 text-center">
                  ...
                  <p className="font-medium">
                    {order.declared_value ? `Rs. ${order.declared_value}` : "-"}
                  </p>
                </div>
              </div>
              {order.seller_name && (
                <div className="pt-2 border-t">
                  ...
                  {order.seller_order_ref && (
                    <p className="text-sm text-muted-foreground">
                      Ref: {order.seller_order_ref}
                    </p>
                  )}
                </div>
              )}
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              {order.route_name && (
                <p className="text-sm">
                  Route: <span className="font-medium">{order.route_name}</span>
                </p>
              )}
              {order.driver_name && (
                <p className="text-sm">
              ...
              {order.delivery_slot && (
                <p className="text-sm">
                  Slot:{" "}
                  <span className="font-medium">{order.delivery_slot}</span>
                </p>
              )}
              ...
              <div className="pt-3 border-t space-y-1">
                ...
                {order.failed_at && (
                  <p className="text-xs text-red-600">
                    Failed: {new Date(order.failed_at).toLocaleString()}
                  </p>
                )}
                {order.returned_at && (
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                <Camera className="h-4 w-4" /> Delivery Attempts (
                  ...
                  {order.attempts.map((attempt) => (
                    <div key={attempt.id} className="rounded border p-3">
                      ...
                      {attempt.recipient_name && (
                        <p className="text-sm text-muted-foreground mt-1">
                          Received by: {attempt.recipient_name}
                        </p>
                      )}
                      ...
                      {attempt.cod_collected && (
                        <p className="text-xs text-amber-600 mt-1">
                          COD Collected: Rs. {attempt.cod_amount}
                        </p>
                      )}
